...
with torch.no_grad():
    for i in range(len(valid_loader.dataset)):
        ...
        if WSOL:
            top_prediction = all_potential_locations_filtered_confidence[0]


        # collect WSOL results statistics
        total_samples   += 1
        iou_all     = region_iou(top_prediction["final_glimpse_loc_and_dim"], target_bbox)
        iou, gt_id  = torch.max(iou_all, dim=1)
        test_ave_iou    += iou.sum().item()
        
        is_correct_label    = top_prediction["prediction_label"] == target_class[gt_id.item()]
        is_correct_box      = (iou >= config_3.iou_th).squeeze()
        
        acc_correct_class   += is_correct_label.sum().item()
        acc_localization    += is_correct_box.sum().item()
        acc_class_localized += torch.logical_and(is_correct_label, is_correct_box).sum().item()
        
        if (i+1) %100 == 0 or i == len(valid_loader.dataset)-1:
            print("{}/{} images: Top-1 Cls: {:.4f} [{}/{}] | GT Loc: {:.4f} [{}/{}] | Top-1 Loc: {:.4f} [{}/{}]\n".format(
                i+1, len(valid_loader.dataset),
