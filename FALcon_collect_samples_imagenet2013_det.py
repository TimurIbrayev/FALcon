    ...
    for i in range(args.start-1, args.end, 1):
        ...
        if len(all_potential_locations) > 0:
            ...
            for idx in nms_objectness_filtered_idx:
                ...
                potential_location["prediction_synset"]         = prediction_synset
                if prediction_synset in valid_loader.dataset.detection_synsets_to_labels.keys():
                    potential_location["prediction_label"] = valid_loader.dataset.detection_synsets_to_labels[prediction_synset]
                else:
                    potential_location["prediction_label"] = 0
                all_potential_locations_filtered_objectness.append(potential_location)
        
        elif len(all_potential_locations) == 0:
            dummy_result = {}
            dummy_result["xyxy_box"] = torch.zeros((4)).to(device)
            dummy_result["prediction_label"] = -1
            dummy_result["prediction_confidence"] = 0.0
